...
import v1.fixtures.auth.AuthFixture
...
class EnrolmentsAuthServiceSpec extends ServiceSpec with AuthFixture {
    ...
    "an exception occurs during enrolment authorisation and v1.nrs check is not required" must {
      "map the exceptions correctly" when {
        ...
        val authServiceErrorMap: Seq[(RuntimeException, MtdError)] =
          Seq(
            (InsufficientEnrolments(), UnauthorisedError),
            (InsufficientConfidenceLevel(), UnauthorisedError),
            (JsResultException(Seq.empty), DownstreamError),
